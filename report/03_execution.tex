...
Every protected asset is assigned a criticality tier by the consequence of its compromise, following the consequence-based approach set out in Section~\ref{sec:criticality} \cite{inl2016cce, cisa2025assetinventory}. Table~\ref{tab:criticality} lists the criticality tiers and their weights. Four tiers are used rather than more or fewer because a four-level severity scale is the established convention: the CVSS qualitative severity scale runs None, Low, Medium, High and Critical, of which the four non-zero ratings are Low, Medium, High and Critical \cite{first2019cvss31}, and industrial functional-safety practice defines four safety-integrity levels, SIL~1 to SIL~4, as the risk reduction required of a safety function, allocated higher for a higher-consequence hazard \cite{iec61508}. CARS follows that granularity and adopts the same names, grading each asset by the consequence of its compromise. This is a different axis from the security levels of IEC~62443, which grade the capability of the attacker a zone must resist rather than the consequence of the asset. Four is also the fewest that separates the distinct consequence classes this deployment presents: a safety-critical controller whose compromise endangers the running process (CRITICAL), an operator panel or secondary controller whose loss is a serious operational disruption (HIGH), a data collector whose loss is recoverable (MEDIUM), and a low-value auxiliary unit (LOW). The weights are the integers three down to zero so that they scale the response linearly and carry no finer numeric meaning than that ordering. The weight does two things in the response. It sets the duration of a block or an isolate, computed as thirty seconds plus fifteen for each unit of weight (the choice of the thirty-second base and the fifteen-second step is justified in Section~\ref{sec:implementation}), so a critical asset holds an attacker out for seventy-five seconds against thirty for a low one, and it brings escalation forward, so a higher-consequence asset reaches a cut sooner. Criticality therefore shapes not only whether CARS acts but how hard and for how long.
