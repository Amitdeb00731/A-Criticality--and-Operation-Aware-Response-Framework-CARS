...
\caption{The CARS architecture and its deployment. The controller decides and governs the fabric over an out-of-band control plane; the three OpenFlow switches enforce in the data plane and carry the protected assets by role and criticality tier; a Snort sensor reads a gateway mirror; and the Factory IO tank closes a hardware-in-the-loop with PLC1. The dashed groups mark which physical node runs each part. Drawn for this report from the deployed system.}
\label{fig:arch}
\end{figure}
...
on packet P at a switch, ingress port p:
    ...
    if a reactive rule matches P.src:              DROP or meter  // an active isolate / block / throttle
    else if P is an established connection:         goto Table 2   // return traffic of an allowed session
...
\hline
\end{tabular}
\caption{The asset-criticality model: each protected asset's consequence tier and the weight that scales the response. The weight sets the duration of a block or isolate to $30 + 15w$ seconds and brings escalation forward at higher weight. The concrete address bindings behind these assets are read live from the \texttt{/cars/criticality} interface (Figure~\ref{fig:criticality}) and given in full in Appendix~\ref{appx:material}.}
...
function SELECT_RESPONSE(tier, weight, flood, count):
    ...
    if source is a shared (NAT) identity:  return BLOCK   // cut the conduit, not every host behind it
...
function ENFORCE(response, source, dest, weight):
    ...
    if response = ISOLATE:
        install drop  matching (src = source)               // quarantine the source on every conduit
    else if response = BLOCK:
        install drop  matching (src = source, dst = dest)   // cut only the offending conduit
...
on hard_timeout expiry of a reactive rule:
    remove the rule and clear the source's offence count    // self-heals unless the attack renews it
\end{lstlisting}
...
CARS is deployed across the machines of the testbed, in the arrangement drawn in Figure~\ref{fig:arch}. The arrangement keeps the two planes apart: the controller, which decides, is separate from the switches, which enforce, and the sensing sits with the fabric so that detection stays close to the traffic while the decision stays central.
