...
The work is built on a hardware testbed rather than an emulation, so that a response can be judged against its effect on a real process. The testbed comprises four machines and two real PLCs, arranged as the two-cell topology of Figure~\ref{fig:topology}. The physical process cell, with its PLC and operator HMI, is shown in Figure~\ref{fig:hardware}.
