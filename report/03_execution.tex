...
\caption[A malicious write's path, armed and disarmed]{The path of a malicious write in each state of the defence. Disarmed, the write crosses the fabric, reaches the critical PLC and drives the process to an unsafe state. Armed, the reactive rule drops it in the fabric, so it never reaches the PLC and the process is held. The armed-versus-disarmed contrast is measured in Chapter~\ref{chap:evaluation}.}
\label{fig:flow}
\end{figure}
...
The controller selects one of seven graded responses and realises it in OpenFlow. ALLOW and MONITOR admit the traffic and record it. THROTTLE attaches a rate-limiting meter to the conduit. DEFLECT rewrites the destination to a honeypot and installs the forward and reverse rules for it. ISOLATE drops all traffic from the offending source at priority 110, and BLOCK drops the single offending conduit at priority 100. REFUSE is reserved for the safety-critical loop and only mirrors and alerts, never enforcing, so that the operator's control of the process can never be cut by the defence. The seven responses, what each does on the wire and when it is selected, are summarised in Table~\ref{tab:ladder}. Of these, DEFLECT is provided by the engine and is exercised only as a forced probe in the evaluation (Section~\ref{sec:evalcrosslayer}, Appendix~\ref{appx:gapevidence}), not driven autonomously by an attack in the campaign; MONITOR is the allow-with-record case rather than an independently selected response. The responses the campaign drives autonomously are ALLOW, THROTTLE, ISOLATE, BLOCK and REFUSE. Every reactive rule carries the cookie \texttt{0x00ca} and a criticality-scaled timeout of $30 + 15w$ seconds, so it expires and self-heals unless the attack persists.
...
DEFLECT & rewrites the destination to a honeypot and installs the forward and reverse rules & the source is to be drawn off to a decoy rather than dropped (provided by the engine; exercised only as a forced probe, not driven autonomously) \\
