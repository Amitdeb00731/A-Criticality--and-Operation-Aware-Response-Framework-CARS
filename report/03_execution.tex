...
\caption[A malicious write's path, armed and disarmed]{The path of a malicious S7 write in each state of the defence. Disarmed, the write crosses the fabric, reaches the critical PLC and drives the process to an unsafe state. Armed, the reactive rule drops it in the fabric, so it never reaches the PLC and the process is held. The armed-versus-disarmed contrast is measured in Chapter~\ref{chap:evaluation}.}
\label{fig:flow}
\end{figure}
