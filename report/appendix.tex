...
\caption{Decision accuracy at scale, confusion matrix over the 2{,}078-case labelled corpus: 2{,}040 true positives, 38 true negatives, no false positives or negatives, 100\% accuracy with a 95\% Wilson confidence interval of $[0.9982, 1.0000]$. Source: \texttt{results/vast/vast.csv}.}
...
\caption{Reaction window against background decision load on the live testbed: the window holds in the tens of milliseconds up to about a thousand operations a second, rising only as the single-threaded controller nears the decision-and-log bench ceiling of sixteen to eighteen thousand a second. Source: \texttt{results/gap1\_live/curve.csv}.}
\label{fig:statload}
\end{figure}
