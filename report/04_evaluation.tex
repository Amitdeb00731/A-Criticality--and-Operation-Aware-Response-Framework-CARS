...
A dedicated harness times the window directly, measuring attack-frame-to-enforcement on a single clock. Over one hundred autonomous trials the mean-time-to-mitigate was 12.6\,ms at the median (95\% confidence interval 12.3 to 12.9\,ms), the 95th percentile 28.0\,ms and the 99th 47.7\,ms; the slowest of the hundred was 50.5\,ms, and every trial ended in an ISOLATE (Figure~\ref{fig:mttmchart}). The reaction is therefore a few milliseconds in the ordinary case. These figures are a single-source measurement and are best read as a best case: the trials drove one attacker against an otherwise quiet detection path. Under a heavy multi-source event the detection bridge, which tails Snort's alert file and posts each alert over HTTP, would queue, and the window would widen; that bridge limit and its remedy, in-memory inter-process communication, are set out in Section~\ref{sec:threats} and the future work. A smaller earlier run occasionally spiked to about one second when detection fell on a poll boundary; that tail is real but rare, none of the hundred trials reached it, and the packets that arrive within any such window are inert.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{chart_mttm.png}
\caption{Reaction window over one hundred autonomous trials, attack frame to enforcement on a single clock. Median 12.6\,ms, 95th percentile 28.0\,ms, 99th 47.7\,ms, maximum 50.5\,ms; every trial ended in an ISOLATE.}
\label{fig:mttmchart}
\end{figure}
