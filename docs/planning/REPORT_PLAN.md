...
- Ch3 Execution (Design and Implementation): testbed architecture and the SDN fabric; the CARS engine three-table pipeline (GUARD anti-spoof, stateful conntrack policy, L2 switch); the rulebook, operation classes, and criticality model; the response ladder; the flow-integrity checker; the Factory IO hardware-in-the-loop process; key design decisions with their trade-offs (for example the reactive cookie hardening, the flood exemption for legitimate high-rate I/O, remediation scope).
